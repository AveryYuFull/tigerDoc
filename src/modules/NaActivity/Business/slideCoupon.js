...
const tpls = `<${getAlias(1)}-slide-coupon
    :moduleData="moduleData"
    @click="click($event)">
</${getAlias(1)}-slide-coupon>`;

const script = `<script>
    import ${getAlias()}SlideCoupon from 'coupon/SlideCoupon';
    export default {
        components: {
            ${getAlias()}SlideCoupon
        },
        data() {
            return {
                ${MODULEDATA.SLIDERCOUPON}
            }
        }
    };
</script>`;
