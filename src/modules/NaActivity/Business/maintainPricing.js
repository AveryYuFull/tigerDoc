...
const tpls = `<${getAlias(1)}-maintain-pricing
    :moduleData="moduleData"
    @click="click($event)">
</${getAlias(1)}-maintain-pricing>`;

const script = `<script>
    import ${getAlias()}MaintainPricing from 'maintainPricing/MaintainPricing';
    export default {
        components: {
            ${getAlias()}MaintainPricing
        },
        data() {
            return {
                ${MODULEDATA.MAINTAINPRICING}
            }
        }
    };
</script>`;
