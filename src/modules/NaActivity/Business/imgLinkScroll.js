...
const tpls = `<${getAlias(1)}-img-link-scroll
    :moduleData="moduleData"
    @click="click($event)">
</${getAlias(1)}-img-link-scroll>`;

const script = `<script>
    import ${getAlias()}ImgLinkScroll from 'imgLinkScroll/ImgLinkScroll';
    export default {
        components: {
            ${getAlias()}ImgLinkScroll
        },
        data() {
            return {
                ${MODULEDATA.IMGLINKSCROLL}
            }
        }
    };
</script>`;
